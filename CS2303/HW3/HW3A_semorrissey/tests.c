...
bool tests(void)
{
	...
	bool ok7 = testMystrndup();
	if(ok7){
		puts("mystrndup() has passed.");
	}
...
bool testMystrndup() {
	...
	char s1[] = "Some sample string."; // String we will duplicate.
	char* s2 = strndup(s1, mystrlen2(s1)); // Copy it using standard function
	char* s3 = mystrndup(s1, mystrlen2(s1)); // Copy it using my function
