...
create table Ship (
    ...
    companyName varchar2(15),
    yearBuilt number(4)  CHECK (yearBuilt >= 1822),
    crew number(4),
    passengers number(4),
    tonnage number(6) CHECK (tonnage >= 50000 and tonnage <= 110000 and mod(tonnage, 15000) != 0),
    dailyTips number(5,2),
    Constraint Ship_companyName_shipName_PK Primary Key (companyName, shipName),
    Constraint Ship_companyName_FK Foreign Key (companyName) References Company(companyName) 
...
create table Cruise (
    ...
    Constraint Cruise_companyName_FK Foreign Key (companyName, shipName) References Ship(companyName, shipName)
        On Delete Cascade
);
...
update Reservation
set travelDate = travelDate + 365;

update Reservation
set paymentDate = 
case
     when travelDate > sysdate then travelDate-120
     else sysdate-120
end;
...
select * from Reservation;
-- 3b
select shipName, companyName, passengers + crew as TotalPeople from Ship
where passengers + crew > 3500 
and companyName in 
(select companyName
from Company 
...
select companyName, shipName, '$' || TO_CHAR(sum(cruiseID*price + days*dailyTips), '999990.99') As "TotalPrice"
from Ship natural join Cruise natural join Reservation
group by rollup (companyName, shipName)
order by companyName, shipName;

-- 3d


select companyName, sum(INDVSUMDAYS) as "SUMDAYS"
from
    (select companyName, days*cruiseIDCOUNT as "INDVSUMDAYS"
     from Cruise natural join
        (select cruiseID, count(*) as cruiseIDCOUNT
            from Reservation
            group by CruiseID)
            )
group by companyName
having sum(INDVSUMDAYS) = 
    (select max(INDVISUMDAYS)
    from(
        select sum(INDVSUMDAYS) as "INDVISUMDAYS"
            from
            (select companyName, days*cruiseIDCOUNT as "INDVSUMDAYS"
            from Cruise natural join
                (select cruiseID, count(*) as cruiseIDCOUNT
